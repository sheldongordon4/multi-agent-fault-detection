...
- **Coordinator (`app/faults/`):** LangGraph ReAct agent (Azure `gpt-4o-mini` via OpenAI-compatible endpoint) with the single `kb_retrieve` tool, **rate-limited + bounded retries + timeout**, and a **local heuristic fallback** when Azure is unconfigured. `FaultTicket` uses `StrEnum` severity/status with tolerant coercion.
...
3. **Fast-path streaming consumer** — the generic consumer caps at ~1 msg/s (pause+`poll(1s)`); the `raw.signals` firehose needs a batch/auto-commit consumer.
4. **Feeder-agnostic classifier** (§6.1) so it transfers across feeders.
5. **Streamlit UI → SSE** (currently reads ticket JSON), and notification email/external channels.
