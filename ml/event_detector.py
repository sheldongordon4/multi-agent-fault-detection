"""
event_detector.py — IsolationForest fault TRIGGER on the testbed event schema.

One row = one fault EVENT across all of a feeder's monitored buses, already
reduced to per-bus features (30 cols on the 13-bus feeder, 42 on the 34-bus).
There is no time axis and no anomaly windows — this is the EVENT-level path,
distinct from the legacy time-series detector in ml/baseline_detector.py.
...
Usage:
    from ml.event_detector import train_isoforest_on_testbed, detect_event
    ...
    # or from the CLI:
    .venv/Scripts/python.exe ml/event_detector.py --csv data/generated/normal_train_013.csv
"""
...
def publish_event(
    ...
    *,
    ...
    for bus in _detect_buses(feature_cols):
        vals = {}
        for fam in ("min_Va_fault_pu", "max_Ia_fault",
                    "max_I0_I1_fault", "max_I2_I1_fault"):
            col = f"{fam}_{bus}"
        ...
        bus_rows.append({
            ...
            "maxIa": round(vals["max_Ia_fault"], 2) if vals["max_Ia_fault"] is not None else None,
            "maxI0I1": round(vals["max_I0_I1_fault"], 4) if vals["max_I0_I1_fault"] is not None else None,
            "maxI2I1": round(vals["max_I2_I1_fault"], 4) if vals["max_I2_I1_fault"] is not None else None,
